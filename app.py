...
# --- Game Settings --- (Keep all your existing settings)
GRID_WIDTH, GRID_HEIGHT, GAME_HEARTBEAT_RATE, SHOUT_MANA_COST, MAX_VIEW_DISTANCE = 20, 15, 0.75, 5, 8
_game_loop_started_in_this_process = False # This flag might become less critical
DESTROY_WALL_MANA_COST = 10
INITIAL_WALL_ITEMS = 77
INITIAL_POTIONS = 77
TILE_FLOOR = 0; TILE_WALL = 1; TILE_WATER = 2
...
# --- Entity Classes (Player, ManaPixie, Scene) ---
# (Keep these classes exactly as they were in the previous complete app.py)
# For brevity, I will omit them here but assume they are present and correct.
class ManaPixie:
    ...
    def get_public_data(self): return {'id': self.id, 'name': self.name, 'char': self.char, 'x': self.x, 'y': self.y, 'scene_x': self.scene_x, 'scene_y': self.scene_y}
    def wander(self, scene):
        if random.random() < 0.3:
            ...
            if 0 <= new_x < GRID_WIDTH and 0 <= new_y < GRID_HEIGHT:
                ...
                if tile_type != TILE_WALL and not scene.is_npc_at(new_x, new_y, exclude_id=self.id): self.x, self.y = new_x, new_y
        ...
        for dx_evade in [-1, 0, 1]:
            for dy_evade in [-1, 0, 1]:
                ...
                if 0 <= evade_x < GRID_WIDTH and 0 <= evade_y < GRID_HEIGHT:
                    ...
                    if tile_type != TILE_WALL and not scene.is_npc_at(evade_x, evade_y, exclude_id=self.id) and not scene.is_player_at(evade_x, evade_y, player_id_to_check=None):
...
class Player:
    def __init__(self, sid, name):
        self.id = sid; self.name = name; self.scene_x = 0; self.scene_y = 0; self.x = GRID_WIDTH // 2; self.y = GRID_HEIGHT // 2
        self.char = random.choice(['^', 'v', '<', '>']); self.max_health = 100; self.current_health = 100
        self.max_mana = 175; self.current_mana = 175.0; self.potions = INITIAL_POTIONS; self.gold = 0; self.walls = INITIAL_WALL_ITEMS
        self.is_wet = False; self.time_became_wet = 0; self.mana_regen_accumulator = 0.0
    def update_position(self, dx, dy, new_char, game_manager, socketio_instance):
        ...
        self.char = new_char
        if scene_changed_flag: game_manager.handle_player_scene_change(self, old_scene_x, old_scene_y)
    ...
    def set_wet_status(self, status, socketio_instance, reason="unknown"):
        if self.is_wet != status: self.is_wet = status
            if status: self.time_became_wet = time.time()
    ...
    def regenerate_mana(self, base_regen_amount, pixie_boost_total, socketio_instance):
        ...
        if self.mana_regen_accumulator >= 1.0: mana_to_add = int(self.mana_regen_accumulator); self.current_mana = min(self.max_mana, self.current_mana + mana_to_add); self.mana_regen_accumulator -= mana_to_add
...
class Scene:
    def __init__(self, scene_x, scene_y, name_generator_func=None):
        ...
        self.players_sids = set(); self.npc_ids = set(); self.terrain_grid = [[TILE_FLOOR for _ in range(GRID_WIDTH)] for _ in range(GRID_HEIGHT)]
        self.is_indoors = False; self.game_manager_ref = None 
...
class GameManager:
    def __init__(self, socketio_instance):
        ...
        self.loop_is_actually_running_flag = True # Start as true, can be set to False to stop
        self.game_loop_greenlet = None # To hold the game loop greenlet
    ...
    def setup_spawn_shrine(self, scene_obj):
        ...
        scene_obj.set_tile_type(mid_x - (shrine_size + 2), mid_y, TILE_WATER); scene_obj.set_tile_type(mid_x - (shrine_size + 2), mid_y + 1, TILE_WATER); scene_obj.set_tile_type(mid_x + (shrine_size + 2), mid_y -1, TILE_WATER)
    def get_or_create_scene(self, scene_x, scene_y):
        scene_coords = (scene_x, scene_y)
        if scene_coords not in self.scenes: new_scene = Scene(scene_x, scene_y); new_scene.game_manager_ref = self; 
    ...
    def add_player(self, sid):
        name = get_player_name(sid); player = Player(sid, name); print(f"[{os.getpid()}] GM Add Player: Creating player {name} ({sid})."); self.players[sid] = player
        scene = self.get_or_create_scene(player.scene_x, player.scene_y); scene.add_player(sid); print(f"[{os.getpid()}] GM Add Player: Added {name} to scene ({player.scene_x},{player.scene_y}). Total players: {len(self.players)}")
        new_player_public_data = player.get_public_data()
        for other_sid_in_scene in scene.get_player_sids():
            if other_sid_in_scene != sid: print(f"[{os.getpid()}] GM Add Player: Notifying {other_sid_in_scene} about new player {name}."); self.socketio.emit('player_entered_your_scene', new_player_public_data, room=other_sid_in_scene)
        return player
    def remove_player(self, sid):
        player = self.players.pop(sid, None); 
        if sid in self.queued_actions: del self.queued_actions[sid]
        if player: old_scene_coords = (player.scene_x, player.scene_y)
            if old_scene_coords in self.scenes: scene = self.scenes[old_scene_coords]; scene.remove_player(sid)
    ...
    def handle_player_scene_change(self, player, old_scene_x, old_scene_y):
        ...
        if old_scene_coords != new_scene_coords:
            if old_scene_coords in self.scenes: old_scene_obj = self.scenes[old_scene_coords]; old_scene_obj.remove_player(player.id)
    ...
    def get_visible_players_for_observer(self, observer_player):
        ...
        for target_sid in scene.get_player_sids():
            ...
            if target_player and self.is_player_visible_to_observer(observer_player, target_player): visible_others.append(target_player.get_public_data())
    ...
    def get_visible_npcs_for_observer(self, observer_player):
        ...
        for npc_id in scene.get_npc_ids():
            ...
            if npc and self.is_npc_visible_to_observer(observer_player, npc): visible_npcs_data.append(npc.get_public_data())
    ...
    def process_sensory_perception(self, player, scene):
        ...
        for npc_id in scene.get_npc_ids():
            ...
            is_visible_flag = self.is_npc_visible_to_observer(player, npc); distance = abs(player.x - npc.x) + abs(player.y - npc.y)
            if is_visible_flag:
                for cue_key, relevance, _ in npc.sensory_cues.get('sight', []):
                    if random.random() < (relevance * 0.05) and cue_key not in perceived_cues_this_tick: self.socketio.emit('lore_message', {'messageKey': cue_key, 'placeholders': {'npcName': npc.name}, 'type': 'sensory-sight'}, room=player.id); perceived_cues_this_tick.add(cue_key); break 
            else: 
                for sense_type in ['sound', 'smell', 'magic']:
                    for cue_key, relevance, cue_range in npc.sensory_cues.get(sense_type, []):
                        if distance <= cue_range:
                            ...
                            if random.random() < perception_chance and cue_key not in perceived_cues_this_tick: self.socketio.emit('lore_message', {'messageKey': cue_key, 'placeholders': {'npcName': npc.name, 'direction': self.get_general_direction(player, npc)}, 'type': f'sensory-{sense_type}'}, room=player.id); perceived_cues_this_tick.add(cue_key); break 
        ...
        for sid_action, action_data in current_actions_to_process.items():
            ...
            if action_type == 'move' or action_type == 'look':
                ...
                if action_type == 'move' and (dx != 0 or dy != 0):
                    target_x, target_y = player.x + dx, player.y + dy; scene_of_player = self.get_or_create_scene(player.scene_x, player.scene_y); can_move_to_tile = True
                    if 0 <= target_x < GRID_WIDTH and 0 <= target_y < GRID_HEIGHT:
                        tile_type_at_target = scene_of_player.get_tile_type(target_x, target_y); npc_at_target = self.get_npc_at(target_x, target_y, player.scene_x, player.scene_y)
                    ...
                    if action_type == 'look': scene_of_player = self.get_or_create_scene(player.scene_x, player.scene_y); self.process_sensory_perception(player, scene_of_player) 
            elif action_type == 'build_wall':
                dx, dy = details.get('dx', 0), details.get('dy', 0); target_x, target_y = self.get_target_coordinates(player, dx, dy); scene = self.get_or_create_scene(player.scene_x, player.scene_y)
...
def game_loop_iteration():
    """Performs a single iteration of the game loop."""
    my_pid = os.getpid() # Get PID inside the function that runs per iteration
    game_manager.loop_iteration_count = getattr(game_manager, 'loop_iteration_count', 0) + 1
    loop_count = game_manager.loop_iteration_count

    # print(f"====== [{my_pid}] TOP OF GAME HEARTBEAT {loop_count} ======") # Verbose
    loop_start_time = time.time()
    
    try: game_manager.process_actions()
    except Exception as e: print(f"!!!!!! [{my_pid}] Heartbeat {loop_count}: EXCEPTION in process_actions: {e} !!!!!!"); traceback.print_exc()
    ...
    except Exception as e: print(f"!!!!!! [{my_pid}] Heartbeat {loop_count}: EXCEPTION in mana_regen: {e} !!!!!!"); traceback.print_exc()

    try:
        if game_manager.server_is_raining:
            for player_obj in list(game_manager.players.values()): 
                player_scene = game_manager.get_or_create_scene(player_obj.scene_x, player_obj.scene_y)
                if not player_scene.is_indoors and not player_obj.is_wet: player_obj.set_wet_status(True, sio, reason="rain")
        for player_obj in list(game_manager.players.values()):
            player_scene = game_manager.get_or_create_scene(player_obj.scene_x, player_obj.scene_y)
            if player_scene.is_indoors and player_obj.is_wet: player_obj.set_wet_status(False, sio, reason="indoors")
    except Exception as e: print(f"!!!!!! [{my_pid}] Heartbeat {loop_count}: EXCEPTION in rain/wetness: {e} !!!!!!"); traceback.print_exc()
    ...
    except Exception as e: print(f"!!!!!! [{my_pid}] Heartbeat {loop_count}: EXCEPTION in sensory: {e} !!!!!!"); traceback.print_exc()
    ...
    try:
        if game_manager.players:
            ...
            if num_updates_sent_this_heartbeat > 0 and loop_count % 10 == 1: print(f"[{my_pid}] Heartbeat {loop_count}: Sent 'game_update' to {num_updates_sent_this_heartbeat} players.")
            elif len(current_players_snapshot) > 0 and num_updates_sent_this_heartbeat == 0 and loop_count % 10 == 1 : print(f"[{my_pid}] Heartbeat {loop_count}: Players present, but NO 'game_update' emitted.")
    except Exception as e: print(f"!!!!!! [{my_pid}] Heartbeat {loop_count}: EXCEPTION in emit section: {e} !!!!!!"); traceback.print_exc()
            
    elapsed_time = time.time() - loop_start_time
    sleep_duration = GAME_HEARTBEAT_RATE - elapsed_time
    
    next_iteration_delay = max(0.0001, sleep_duration) # Ensure at least a tiny delay to yield
    
    if game_manager.loop_is_actually_running_flag: # Check flag before scheduling next
        eventlet.spawn_after(next_iteration_delay, game_loop_iteration) # Schedule the next iteration
    else:
        print(f"[{my_pid}] Heartbeat {loop_count}: loop_is_actually_running_flag is False. Loop will not reschedule.")
    
    # print(f"====== [{my_pid}] BOTTOM OF GAME HEARTBEAT {loop_count} - Next iter in {next_iteration_delay:.4f}s ======\n")


def master_game_loop_starter():
    """This function is called once by start_background_task. It then calls the iterating function."""
    my_pid = os.getpid()
    print(f">>>> [{my_pid}] master_game_loop_starter: Initializing game loop and NPCs... <<<<")
    if not hasattr(game_manager, 'loop_is_actually_running_flag'):
        game_manager.loop_is_actually_running_flag = False # Initialize if not present
    
    game_manager.loop_is_actually_running_flag = True # Set it to true before first call
    game_manager.spawn_initial_npcs()
    print(f"[{my_pid}] master_game_loop_starter: Initial NPCs spawned. Starting first game_loop_iteration.")
    game_loop_iteration() # Start the first iteration
...
def handle_disconnect_event():
    ...
    else: print(f"[{pid}] Disconnect for SID {sid} (player not found/removed).")
...
def start_game_loop_for_worker():
    ...
    if not _game_loop_started_in_this_process:
        print(f"[{my_pid}] Worker: Attempting to start game_loop task via master_game_loop_starter...")
        try:
            # Instead of sio.start_background_task(target=game_loop),
            # we use eventlet.spawn to start a function that will then call the iterating loop.
            game_manager.loop_is_actually_running_flag = True # Ensure flag is true before spawning
            game_manager.game_loop_greenlet = eventlet.spawn(master_game_loop_starter)
            _game_loop_started_in_this_process = True
            # sio.sleep(0.01) # Small yield to let the spawned task start
            print(f"[{my_pid}] Worker: Game loop greenlet spawned via eventlet.spawn(master_game_loop_starter).")
        except Exception as e:
            print(f"!!! [{my_pid}] Worker: FAILED TO START GAME LOOP VIA EVENTLET.SPAWN: {e} !!!")
            traceback.print_exc()
    else: 
